...
const tagline = content.match(/Short tagline options:\s*\n\n-\s*(.*)/)?.[1]?.trim() || 'AI dispatch app for small field-service teams';

// Extract short description
const shortDescription = content.match(/## Short description\s*\n\n(.*)/)?.[1]?.trim() || '';
// Extract longer description
const longDescription = content.match(/## Longer directory description\s*\n\n(.*)/)?.[1]?.trim() || '';
...
async function pauseForHuman(message) {
  console.log(`\n👉 ${message}`);
  if (headless) {
    console.log('Running in headless/non-interactive mode. Proceeding automatically in 3 seconds...');
    await new Promise(resolve => setTimeout(resolve, 3000));
  } else {
    await askQuestion('Press Enter in this terminal after you have resolved this step to continue...');
  }
}
...
async function fillGenericFields(page, meta) {
  const inputs = [
    { selectors: ['input[name*="name" i]', 'input[placeholder*="name" i]', 'input[placeholder*="Product" i]'], value: meta.productName },
    { selectors: ['input[name*="url" i]', 'input[name*="website" i]', 'input[placeholder*="URL" i]', 'input[placeholder*="Website" i]'], value: meta.website },
    { selectors: ['input[name*="email" i]', 'input[placeholder*="email" i]'], value: meta.email },
    { selectors: ['input[name*="tagline" i]', 'input[name*="title" i]', 'input[placeholder*="tagline" i]'], value: meta.tagline },
    { selectors: ['textarea[name*="description" i]', 'textarea[name*="desc" i]', 'textarea[placeholder*="Description" i]'], value: meta.longDescription }
  ];
...
async function main() {
  ...
  console.log('\n--- SaaSHub ---');
  try {
    await page.goto('https://www.saashub.com/submit', { timeout: 30000, waitUntil: 'domcontentloaded' });
  ...
  console.log('\n--- BetaList ---');
  try {
    await page.goto('https://betalist.com/submit', { timeout: 30000, waitUntil: 'domcontentloaded' });
    await fillGenericFields(page, metadata);
  ...
  console.log('\n--- Futurepedia ---');
  try {
    await page.goto('https://www.futurepedia.io/submit-a-tool', { timeout: 30000, waitUntil: 'domcontentloaded' });
    await fillGenericFields(page, metadata);
  ...
  try {
    await page.goto('https://theresanaiforthat.com/submit/', { timeout: 30000, waitUntil: 'domcontentloaded' });
    await fillGenericFields(page, metadata);
  ...
  console.log('\n--- Startup Buffer ---');
  try {
    await page.goto('https://startupbuffer.com/site/submit', { timeout: 30000, waitUntil: 'domcontentloaded' });
    await fillGenericFields(page, metadata);
