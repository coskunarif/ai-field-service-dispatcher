...
async function main() {
  const outputDir = join(process.cwd(), 'dogfood-output', 'scout-20260622-run2', 'screenshots');
  ...
  await page.screenshot({ path: join(outputDir, 'desktop-homepage.png') });
  
  await mPage.goto(`http://localhost:${port}/`, { waitUntil: 'networkidle' });
  await mPage.screenshot({ path: join(outputDir, 'mobile-homepage.png') });

  // 2. HVAC Software Page Desktop & Mobile
  console.log('Capturing HVAC page...');
  await page.goto(`http://localhost:${port}/hvac-dispatch-software`, { waitUntil: 'networkidle' });
  await page.screenshot({ path: join(outputDir, 'desktop-hvac-landing.png') });

  await mPage.goto(`http://localhost:${port}/hvac-dispatch-software`, { waitUntil: 'networkidle' });
  await mPage.screenshot({ path: join(outputDir, 'mobile-hvac-landing.png') });
  ...
  serverProc.kill();
  console.log(`Screenshots captured successfully inside dogfood-output/scout-20260622-run2/screenshots/`);
}
