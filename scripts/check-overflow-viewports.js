...
try {
  ...
  for (const vp of viewports) {
    ...
    for (const path of pages) {
      const url = `http://localhost:3009${path}`;
      ...
      const overflowInfo = await page.evaluate(() => {
        ...
        if (overflow) {
          ...
          for (const el of all) {
            // Skip elements that are display none or height/width 0
...
writeFileSync('scripts/overflow-results.json', JSON.stringify(results, null, 2));
console.log('Results written to scripts/overflow-results.json');
