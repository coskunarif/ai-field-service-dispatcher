...
test.describe('Gainhelm Dual-Screen Sandbox Simulator E2E', () => {
  test('runs happy path dispatch simulation (accept job)', async ({ page }) => {
    ...
    await expect(phoneBody).toContainText('Job confirmed! Scheduled in Google Calendar', {
      timeout: 8000,
    });
  ...
  test('runs fallback retry path on unsupported response (running late)', async ({ page }) => {
    ...
    await expect(phoneBody).toContainText(
      'Sorry, I did not catch that. Reply YES to accept the dispatch, or BUSY to decline.',
      { timeout: 8000 }
    );
    await expect(terminal).toContainText('Received reply from Sarah Connor: "LATE"', {
      timeout: 8000,
    });
