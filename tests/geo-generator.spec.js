...
test.describe('GEO Generator Module Integration Tests', () => {
    ...
    test.describe('page_generator', () => {
        ...
        test('should reject invalid service types', async () => {
            await expect(page_generator('INVALID_TRADE', ['software'])).rejects.toThrow(/invalid service type/i);
        });

        test('should reject keywords with unsafe characters', async () => {
            await expect(page_generator('HVAC', ['<script>alert(1)</script>'])).rejects.toThrow(/invalid characters in keywords/i);
        });
